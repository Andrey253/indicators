...
counted_bars=IndicatorCounted();
      ...
      { 
          ...
          for (k=0;k<por;k++)
             {
               x[z][k*2+0]=(Low  [cm+k+z]);
               x[z][k*2+1]=(High [cm+k+z]);
             }
      }  
    for (p=0;p<(por*2-1);p++)
      {
      for (n=p;n<(por*2-1);n++)
         {if (x[p][p]!=0) mn=x[n+1][p]/x[p][p];
          for (m=p;m<por*2;m++){x[n+1][m]=x[n+1][m]-x[p][m]*mn; }
              y[n+1]=y[n+1]-y[p]*mn;
         }
       }
        for (k=(por*2-1);k>=0;k--)
           { xx[k]=(y[k]-pr[k])/x[k][k];for (z=k;z<=(por*2-1);z++) pr[k-1]=pr[k-1]+x[k-1][z]*xx[z]; }
   /////////////////////////////////////
         for (z=0;z<por*2;z++)
      { 
          y[z]=(Low [cm+z-1]);
    ...
    for (p=0;p<(por*2-1);p++)
      ...
      for (n=p;n<(por*2-1);n++)
        ...
        for (k=(por*2-1);k>=0;k--)
           { xxl[k]=(y[k]-prl[k])/x[k][k];for (z=k;z<=(por*2-1);z++) prl[k-1]=prl[k-1]+x[k-1][z]*xxl[z]; }
   /////////////////////////////////   
   i=Bars-counted_bars-por*2-5;
   while(i>0)
     {
     f=0;
    for (a=0;a<por;a++)
